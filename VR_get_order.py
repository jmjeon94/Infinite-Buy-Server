...
RATIO_RISE = [1,1.001,1.005,1.01,1.015,1.02,1.025,1.03,1.035,1.04,1.045,1.05,1.055,1.06,1.065,1.07,1.075,1.08,1.085,1.09,1.095,1.1,1.105,1.11,1.115]
...
def get_expect_value(n, post_expect_value):
    ...
    if pool/post_expect_value <= 0.01:
        ...
        number = int(pool/real_value/0.05)+1
...
def make_order():
    ...
    while True: # 매수 주문서 작성
        price = round(min_value/n,2)

        pool_1 -= price

        if pool_1 < qurter_pool:
            break

        else:
            buy_list.append({'n':int(n+1), 'price':price, 'pool':round(pool_1,2)})
            n += 1

    i = 0
    while i < 12: # 매도 주문서 작성
        price = round(max_value / n, 2)

        pool_2 += price

        sell_list.append({'n': int(n - 1), 'price': price, 'pool': round(pool_2,2)})
        n -= 1
        i += 1
